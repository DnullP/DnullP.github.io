...
<span class="token punctuation">}</span>
 ...
 .def main; .scl 2; .type 32; .endef
 .seh_proc main
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其中的 <code>movl $0, %edx</code>被换成了 <code>cqto</code>，这条指令的意思是把 <code>%rax</code>符号扩展到16位，高八位储存在 <code>%rdx</code>中，这样来实现有符号数的符号扩展，而除法指令改变为使用 <code>idivq</code></p><h3 id="条件码及其设置" tabindex="-1"><a class="header-anchor" href="#条件码及其设置"><span>条件码及其设置</span></a></h3><p>除去之前一直提到整数寄存器，除去整数寄存器处理器中还有一部分被称为<strong>条件码寄存器</strong>，里面记录了一系列数字，用来表示最近一次运算的一些性质，这些数字被称为条件码，下面是书上记录的常用条件码：</p><figure><img src="`+o+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>除了之前的常用运算指令会设置条件码，还有两组指令 <code>CMP</code>和 <code>TEST</code>：</p><figure><img src="'+r+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><ul><li><code>CMP</code>指令将两个数相减，后者减去前者</li><li><code>TEST</code>指令将两个数执行与（&amp;）操作</li></ul><h4 id="访问条件码" tabindex="-1"><a class="header-anchor" href="#访问条件码"><span>访问条件码</span></a></h4><p>第一种方式：<strong>SET</strong>指令</p><p><code>SET</code>指令会根据当前的条件码状态，将一个目标设置为0或1，用于得到上次运算的某些特点</p><figure><img src="'+u+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>简单记忆一下，e相等，g更大，l更小，ge大等，le小等，ab大小无符号</p><div class="language-asm line-numbers-mode" data-ext="asm" data-title="asm"><pre class="language-asm"><code>comp:
 .seh_endprologue
 cmpl %edx, %ecx
 setg %al
 ret
...
.L5:
 ...
 .def main; .scl 2; .type 32; .endef
 .seh_proc main
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这之中，<code>.L8</code>这样的标记也表示一个内存位置，通过一系列计算得到需要跳转的位置，通过*号来间接寻址，得到跳转位置</p><h3 id="过程" tabindex="-1"><a class="header-anchor" href="#过程"><span>过程</span></a></h3><p><strong>过程</strong>是种程序的抽象，比较容易理解的例子就是函数，此外还包括<strong>方法</strong>、<strong>子例程</strong>等等</p><p>函数之间的互相调用就形成了过程之间的嵌套，比如过程A中需要跳转到另一个过程B，完成过程B后再跳回过程A继续执行。</p><p>我们知道CPU寄存器的空间是有限的，如果过程中的变量数超过了寄存器的空间，就会把多出来的数据写到栈里面，这个栈被称为<strong>运行时栈</strong>，栈中每个分配的数据被称为<strong>栈帧</strong></p><p>此外，在过程的互相跳转中，如果要跳转到过程B，那么需要保存过程A此时的数据和程序运行到的内存位置，在完成B过程后再跳转回来，这些程序的控制信息也是保存在运行时栈中的</p><h3 id="call和ret指令" tabindex="-1"><a class="header-anchor" href="#call和ret指令"><span>call和ret指令</span></a></h3><p><code>call</code>指令操作数可以是标号或者地址，把下一条指令的地址压入栈，然后使PC指向操作数的地址</p><p><code>ret</code>出栈并跳转回原来的过程，此时，如果有返回值，返回值储存在 <code>%rax</code>寄存器中</p><p>传递参数时，如果参数数量小于等于6，则可以直接使用寄存器传递参数，具体参数的位置按照参数的大小如下：</p><figure><img src="`+b+'" alt="avatar" tabindex="0" loading="lazy"><figcaption>avatar</figcaption></figure><h3 id="使用栈传递参数" tabindex="-1"><a class="header-anchor" href="#使用栈传递参数"><span>使用栈传递参数</span></a></h3><figure><img src="'+k+`" alt="avatar" tabindex="0" loading="lazy"><figcaption>avatar</figcaption></figure><p>上面的代码中，函数 <code>proc()</code>有八个参数，其中六个以寄存器的方式传递，多出来的两个参数在调用函数时，提前先压入栈中，然后再压入跳转地址，跳转到proc函数所在位置</p><p>此时访问栈中的参数，使用栈顶寄存器进行运算得到： <code>8(%rsp)</code>从 <code>%rsp</code>的前八个字节开始取数据， <code>%rsp</code>即为栈顶寄存器</p><h3 id="把数据压入栈的情况" tabindex="-1"><a class="header-anchor" href="#把数据压入栈的情况"><span>把数据压入栈的情况</span></a></h3><p>除了程序需要把大于6个的参数压入栈中，用来传递给调用的函数，还有以下情况也需要把数据存入运行时栈中：</p><ul><li>寄存器空间不够存本地数据</li><li>使用&amp;符号获得本地变量的地址</li><li>使用数组这种使用索引访问连续数据的</li></ul><p>以第二种情况为例:</p><div class="language-c line-numbers-mode" data-ext="c" data-title="c"><pre class="language-c"><code><span class="token keyword">long</span> <span class="token function">swap_add</span><span class="token punctuation">(</span><span class="token keyword">long</span> <span class="token operator">*</span>xp<span class="token punctuation">,</span> <span class="token keyword">long</span> <span class="token operator">*</span>yp<span class="token punctuation">)</span>
<span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
 ...
 popq %rbx
 ret
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面函数对应的汇编代码中，分配了48字节的栈空间，根据栈帧的结构，参数空间为0，返回地址为8字节，本地变量为4字节，有36字节的未使用空间</p><p>如果输入的字符串超过了36+4个字节，那么多出来的字节就会覆盖返回地址，从而使函数跳转到意料之外的函数中，或者直接无法继续进行。</p><p>使用 <code>gets()</code>这样的函数，如果被人故意使用这样的漏洞，可能使对方调用指定的函数来侵入系统。</p><h4 id="防治方法" tabindex="-1"><a class="header-anchor" href="#防治方法"><span>防治方法</span></a></h4><ul><li><p>栈随机化 每次程序运行分配栈的地址都不一样，这就使得攻击代码没办法准确定位 但是通过在攻击代码前跟上一长段<strong>nop</strong>指令，就能增加攻击代码命中的范围，再通过暴力的方式尝试跳转，可以有更高的几率使攻击代码生效，该方式被称为空操作雪橇（nop sled）</p></li><li><p>栈破坏检测 在局部变量和返回地址之间加入一个随机产生的数据，被称为<strong>金丝雀值</strong>。在调用完函数后检测这些值是否被修改，如果被修改，则说明发生了缓冲区溢出，程序会调用一个异常处理机制来结束程序</p></li><li><p>限制可执行代码区域 以前的内存中，可读区域和可执行区域是一体的，一个可以读写的区域也就可以执行。之后通过把读写区域和执行区域分开来，在不影响性能的情况下避免攻击代码的执行</p></li></ul><h3 id="变长栈帧" tabindex="-1"><a class="header-anchor" href="#变长栈帧"><span>变长栈帧</span></a></h3><p>在栈空间分配大小不确定时，会保存一个帧指针，用来作为访问局部变量的<strong>基址</strong>，因为此时的栈指针位置不能确定了，而帧指针在函数调用后的一开始就创建好，位置是固定的</p><p>帧指针一般保存在 <code>%rbx</code>寄存器中</p><h3 id="浮点代码" tabindex="-1"><a class="header-anchor" href="#浮点代码"><span>浮点代码</span></a></h3><p>CPU中对于浮点数的计算等操作有专门的架构和指令集</p><p>这些体系结构原本是允许多个操作以并行模式执行，来支持图形和图像处理，称为<strong>单指令多数据</strong>（SIMD），最开始的指令集为MXX，后来是SSE，再后来是AVX，还有SSE2和AVX2等</p><p>AVX浮点体系结构允许数据储存在16个YMM寄存器中：</p><figure><img src="`+x+'" alt="avatar" tabindex="0" loading="lazy"><figcaption>avatar</figcaption></figure><p>YMM寄存器是AVX体系结构的寄存器，有256位的空间，xmm是SSE体系的寄存器，有128位空间</p><p>一个YMM寄存器可以储存4个双精度浮点数，或者8个整数，并且通过<strong>向量</strong>操作同时对多个数执行同一个操作，实现<strong>硬件加速</strong></p><p>但是书上说接下来的程序里大部分都只会进行<strong>标量</strong>的计算，也就是说只使用YMM寄存器的低位进行操作和计算</p><h4 id="指令" tabindex="-1"><a class="header-anchor" href="#指令"><span>指令</span></a></h4><figure><img src="'+y+'" alt="avatar" tabindex="0" loading="lazy"><figcaption>avatar</figcaption></figure><p>GCC只会使用标量传送操作来在XMM寄存器和内存之间传送数据，在XMM寄存器之间传输数据时，采用标量或矢量传送其中一种</p><p>因为 <code>vmovaps</code>中的a代表alined（对齐的），所以如果需要把寄存器中的数据传送到内存上，需要保证内存的十六进制对齐，否则发生报错</p><figure><img src="'+f+`" alt="avatar" tabindex="0" loading="lazy"><figcaption>avatar</figcaption></figure><p>这些是实现数据转换的指令，表中的X表示必须是XMM寄存器，R为通用寄存器，M为内存位置</p><p>3-48中的指令是三操作数的指令，一般源2和目的操作数保持一致即可，源2的值只影响结果的高位字节</p><p>书上说把一个单精度值转化为一个双精度值之前，会先使用一次 <code>vunpcklps</code>指令，并且书上表示这个操作并没有任何作用，然后咱尝试这编译了这样的代码：</p><div class="language-c line-numbers-mode" data-ext="c" data-title="c"><pre class="language-c"><code><span class="token keyword">void</span> <span class="token function">test_3</span><span class="token punctuation">(</span><span class="token keyword">double</span> <span class="token operator">*</span>p<span class="token punctuation">,</span> <span class="token keyword">float</span> n<span class="token punctuation">)</span>
<span class="token punctuation">{</span>
